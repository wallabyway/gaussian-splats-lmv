/**
 * Gaussian Splat Renderer for LMV (Three.js R71)
 * Adapted from references/lcc-decoder/splat-renderer.js
 *
 * R71 adaptations:
 *   - window.THREE instead of import
 *   - BufferGeometry + numInstances instead of InstancedBufferGeometry
 *   - BufferAttribute + divisor=1 instead of InstancedBufferAttribute
 */

const THREE = window.THREE;

const MAX_CUT_PLANES = 6;
...
uniform int cutPlaneCount;
uniform vec4 cutPlanes[${MAX_CUT_PLANES}];
varying vec3 v_col;
varying vec4 v_con_o;
varying vec2 v_xy, v_pixf;

vec3 computeCov2D(vec3 mean, float cov3D[6]) {
    vec4 t = modelViewMatrix * vec4(mean, 1.0);
    ...
    mat3 V = mat3(modelViewMatrix[0][0], modelViewMatrix[1][0], modelViewMatrix[2][0],
...
export class GaussianSplatRenderer {
    ...
    async init(data) {
        ...
        this.geometry.setAttribute('a_center', makeInstancedAttr(new Float32Array(n * 3), 3));
        this.geometry.setAttribute('a_col', makeInstancedAttr(new Float32Array(n * 3), 3));
        this.geometry.setAttribute('a_opacity', makeInstancedAttr(new Float32Array(n), 1));
        this.geometry.setAttribute('a_covA', makeInstancedAttr(new Float32Array(n * 3), 3));
        this.geometry.setAttribute('a_covB', makeInstancedAttr(new Float32Array(n * 3), 3));

        const defaultPlanes = [];
        for (let i = 0; i < MAX_CUT_PLANES; i++)
            defaultPlanes.push(new THREE.Vector4(0, 0, 0, 0));
        ...
        this.material = new THREE.ShaderMaterial({
            uniforms: {
                ...
                tan_fovy: { type: 'f', value: 0 },
                cullRadius: { type: 'f', value: 0 },
                cutPlaneCount: { type: 'i', value: 0 },
                cutPlanes: { type: 'v4v', value: defaultPlanes }
            },
            vertexShader,
            fragmentShader,
            transparent: true,
        ...
        this.worker.onmessage = e => {
            ...
            attrs.a_center.array.set(e.data.positions);
            attrs.a_col.array.set(e.data.colors);
            attrs.a_opacity.array.set(e.data.opacities);
            attrs.a_covA.array.set(e.data.covA);
            attrs.a_covB.array.set(e.data.covB);
            attrs.a_center.needsUpdate = true;
            attrs.a_col.needsUpdate = true;
            attrs.a_opacity.needsUpdate = true;
            attrs.a_covA.needsUpdate = true;
            attrs.a_covB.needsUpdate = true;
        };
    ...
    updateBuffers() {
        ...
        attrs.a_center.array.set(this.positions);
        attrs.a_col.array.set(this.colors);
        attrs.a_opacity.array.set(this.opacities);
        attrs.a_covA.array.set(this.cov3DsA);
        attrs.a_covB.array.set(this.cov3DsB);
    ...
    setCutPlanes(planes) {
        ...
        const u = this.material.uniforms;
        const count = Math.min(planes.length, MAX_CUT_PLANES);
